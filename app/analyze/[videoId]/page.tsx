...
const GUEST_LIMIT_MESSAGE =
  "You've used your free preview. Create a free account for 3 videos/month.";
const AUTH_LIMIT_MESSAGE =
  "You've used all 3 free videos this month. Upgrade to Pro for 100 videos/month.";
...
function buildLimitExceededMessage(
  limitData?: LimitCheckResponse | null,
): string {
  if (!limitData) {
...
function normalizeErrorMessage(
  ...
  fallback: string = DEFAULT_CLIENT_ERROR,
): string {
...
function buildApiErrorMessage(errorData: unknown, fallback: string): string {
  ...
  const creditsMessage =
    typeof record.creditsMessage === "string" &&
    record.creditsMessage.trim().length > 0
  ...
  const alreadyIncludes = baseMessage
    .toLowerCase()
    .includes(creditsMessage.toLowerCase());
...
export default function AnalyzePage() {
  ...
  const routeVideoId = Array.isArray(params?.videoId)
    ? params.videoId[0]
    : params?.videoId;
  ...
  const forceRegenerate =
    regenParam?.toLowerCase() === "1" || regenParam?.toLowerCase() === "true";
  ...
  const baseTopicKeySet = useMemo(() => {
    ...
    baseTopics.forEach((topic) => {
      if (topic.quote?.timestamp && topic.quote.text) {
        keys.add(
          `${topic.quote.timestamp}|${normalizeWhitespace(topic.quote.text)}`,
        );
      }
    });
    return keys;
  ...
  const [switchingToLanguage, setSwitchingToLanguage] = useState<string | null>(
    null,
  );
  ...
  const [playbackCommand, setPlaybackCommand] =
    useState<PlaybackCommand | null>(null);
  const [transcriptHeight, setTranscriptHeight] = useState<string>("auto");
  const [citationHighlight, setCitationHighlight] = useState<Citation | null>(
    null,
  );
  ...
  const memoizedSetPlayAllIndex = useCallback(
  ...
  const translateWithContext: TranslationRequestHandler = useCallback(
    (text: string, cacheKey: string, scenario?, targetLanguage?) => {
      return handleRequestTranslation(
        text,
        cacheKey,
        scenario,
        videoInfo,
  ...
  useEffect(() => {
    ...
    const fallbackTitle =
      videoInfo?.title || `YouTube Video ${effectiveVideoId}`;
  ...
  const [authModalTrigger, setAuthModalTrigger] =
    useState<AuthModalTrigger>("generation-limit");

  // Store current video data in sessionStorage before auth
  const storeCurrentVideoForAuth = useCallback(
    (id?: string) => {
  ...
  const checkPendingVideoLink = useCallback(
    async (retryCount = 0) => {
      ...
      if (videoToLink && user) {
        ...
        try {
          ...
          if (response.ok) {
            ...
            console.log(
              `Video not found, retrying in ${1000 * (retryCount + 1)}ms...`,
            );
            setTimeout(
              () => {
            ...
            console.log(
              `Profile not ready, retrying in ${2000 * (retryCount + 1)}ms...`,
            );
            setTimeout(
              () => {
                checkPendingVideoLink(retryCount + 1);
  ...
  const checkRateLimit =
    useCallback(async (): Promise<LimitCheckResponse | null> => {
      try {
  ...
  useEffect(() => {
    if (
      user &&
      !hasAttemptedLinking.current &&
      (videoId || sessionStorage.getItem("pendingVideoId"))
    ) {
  ...
  const normalizedUrl =
    urlParam ??
    (routeVideoId ? `https://www.youtube.com/watch?v=${routeVideoId}` : "");
  ...
  const processVideo = useCallback(
    async (
      ...
      try {
        ...
        const shouldSkipCacheForLanguage =
        ...
        if (!forceRegenerate && !shouldSkipCacheForLanguage) {
          ...
          if (cacheResponse.ok) {
            ...
            if (cacheData.cached) {
              const sanitizedTranscript = normalizeTranscript(
                cacheData.transcript,
              );
              const flattenedTranscript = isFlattenedTranscript(
                sanitizedTranscript,
                cacheData.videoInfo,
              );
              ...
              if (!flattenedTranscript) {
                ...
                if (cachedVideoInfo) {
                  ...
                  const rawDuration = (
                    cachedVideoInfo as { duration?: number | string | null }
                  ).duration;
                ...
                setSelectedTopic(
                  hydratedTopics.length > 0 ? hydratedTopics[0] : null,
                );
                ...
                backgroundOperation(
                  ...
                  (error) => {
                    console.error(
                      "Failed to generate themes for cached video:",
                      error,
                    );
                  },
                );
                        ...
                        setVideoInfo((prev) =>
                          prev
                            ? {
                                ...
                                availableLanguages:
                                  availableLanguages ?? prev.availableLanguages,
                              }
                            : null,
                        );
                      }
                ...
                if (!cacheData.summary) {
                  ...
                  backgroundOperation(
                    ...
                    async () => {
                      ...
                      if (summaryRes.ok) {
                        const { summaryContent: generatedTakeaways } =
                        ...
                        await backgroundOperation(
                          ...
                          async () => {
                            const res = await csrfFetch.post(
                              "/api/update-video-analysis",
                              {
                                videoId: extractedVideoId,
                    ...
                    (error) => {
                      setTakeawaysError(
                        error.message ||
                          "Failed to generate takeaways. Please try again.",
                      );
                    },
                  ).finally(() => {
                    setIsGeneratingTakeaways(false);
                ...
                console.warn(
                  "Cached transcript looks flattened; re-running full analysis.",
                );
              }
            }
          }
        ...
        const transcriptController = abortManager.current.createController(
          "transcript",
          300000,
        );
        const videoInfoController = abortManager.current.createController(
          "videoInfo",
          100000,
        );
        ...
        const transcriptPromise = fetch("/api/transcript", {
          ...
          throw new Error(
            "Network error: Unable to fetch transcript. Please ensure the server is running.",
          );
        });
        ...
        try {
          ...
          if (jsonError instanceof Error && jsonError.name === "AbortError") {
            throw new Error(
              "Transcript processing timed out. The video may be too long. Please try again.",
            );
          }
          throw new Error(
            "Failed to process transcript data. Please try again.",
          );
        }
        ...
        if (!fetchedVideoInfo) {
          setVideoInfo((prev) =>
            prev ? { ...prev, language, availableLanguages } : null,
          );
        }
        ...
        if (transcriptDuration !== undefined && fetchedVideoInfo?.duration) {
          ...
          if (coverageRatio < 0.5) {
            console.warn(
              "[Transcript] WARNING: Transcript may be incomplete!",
              {
                ...
                message:
                  "The transcript covers less than 50% of the video duration. This may indicate an issue with caption availability.",
              },
            );
          }
        }
        ...
        const topicsController =
          abortManager.current.createController("topics");
        const takeawaysController = abortManager.current.createController(
          "takeaways",
          60000,
        );
        ...
        const topicsPromise = fetch("/api/video-analysis", {
          ...
          if (err.name === "AbortError") {
            throw new Error(
              "Topic generation was canceled or interrupted. Please try again.",
            );
          }
          throw new Error(
            "Network error: Unable to generate topics. Please check your connection.",
          );
        });
        ...
        if (!topicsRes.ok) {
          const errorData = await topicsRes
            .json()
            .catch(() => ({ error: "Unknown error" }));
          ...
          if (
            requiresAuth ||
            topicsRes.status === 401 ||
            topicsRes.status === 403
          ) {
            takeawaysController.abort();
            await takeawaysSettledPromise;
            redirectToAuthForLimit(authMessage, extractedVideoId);
            return;
          }
        ...
        const rawTopics = Array.isArray(topicsData.topics)
          ? topicsData.topics
          : [];
        const generatedTopics: Topic[] = hydrateTopicsWithTranscript(
          rawTopics,
          normalizedTranscriptData,
        );
        const generatedThemes: string[] = Array.isArray(topicsData.themes)
          ? topicsData.themes
          : [];
        const rawCandidates: TopicCandidate[] = Array.isArray(
          topicsData.topicCandidates,
        )
          ? topicsData.topicCandidates
          : [];
        const generatedCandidates: TopicCandidate[] = rawCandidates.map(
          (candidate) => ({
            ...candidate,
        ...
        if (takeawaysResult.status === "fulfilled") {
          ...
          if (summaryRes.ok) {
            ...
            const errorData = await summaryRes
              .json()
              .catch(() => ({ error: "Unknown error" }));
            takeawaysGenerationError = buildApiErrorMessage(
              errorData,
              "Failed to generate takeaways. Please try again.",
            );
          }
          ...
          if (error && error.name === "AbortError") {
            takeawaysGenerationError =
              "Takeaways generation timed out. The video might be too long.";
          } else {
            takeawaysGenerationError =
              error?.message ||
              "Failed to generate takeaways. Please try again.";
        ...
        setSelectedTopic(
          generatedTopics.length > 0 ? generatedTopics[0] : null,
        );
        ...
        backgroundOperation(
          ...
          (error) => {
            console.error(
              "Failed to confirm cached analysis for sharing:",
              error,
            );
          },
        );
        ...
        backgroundOperation(
          ...
          async () => {
            ...
            try {
              ...
              console.error(
                "Failed to parse suggested questions payload:",
                error,
              );
              return applyCachedQuestions(buildSuggestedQuestionFallbacks(3));
            }
            ...
            await backgroundOperation("update-questions", async () => {
              const updateRes = await csrfFetch.post(
                "/api/update-video-analysis",
                {
                  videoId: extractedVideoId,
  ...
  useEffect(() => {
    ...
    if (!user) {
      ...
      console.log(
        "Stored route video ID for potential post-auth linking:",
        routeVideoId,
      );
    }
  ...
  const handleTimestampClick = (
    ...
    _citationText?: string,
    isWithinHighlightReel: boolean = false,
    isWithinCitationHighlight: boolean = false,
  ) => {
  ...
  const handleTopicSelect = useCallback(
    (topic: Topic | null, fromPlayAll: boolean = false) => {
    ...
    async (themeLabel: string | null) => {
      ...
      const needsHydration =
        ...
        themedTopics.some((topic) => {
          const firstSegment = Array.isArray(topic?.segments)
            ? topic.segments[0]
            : null;
          return (
            !firstSegment ||
      ...
      const pendingRequestId =
        pendingThemeRequestsRef.current.get(normalizedTheme);
      ...
      if (!themedTopics) {
        ...
        const exclusionKeys = Array.from(baseTopicKeySet).map((key) =>
          key.slice(0, 500),
        );
        ...
        try {
          ...
          if (!response.ok) {
            ...
            const message = buildApiErrorMessage(
              errorData,
              "Failed to generate themed topics",
            );
            throw new Error(message);
          }
          ...
          const hydratedThemeTopics = hydrateTopicsWithTranscript(
            Array.isArray(data.topics) ? data.topics : [],
            transcript,
          );
          const candidatePool = Array.isArray(data.topicCandidates)
  ...
  useEffect(() => {
    const adjustRightColumnHeight = () => {
      const videoContainer = document.getElementById("video-container");
      const rightColumnContainer = document.getElementById(
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      EXPLAIN_SELECTION_EVENT,
      handleExplainFromSelection as EventListener,
    );
    return () => {
  ...
  const handleTakeNoteFromSelection = useCallback(
  ...
  const handleSaveEditingNote = useCallback(
    async ({
      noteText,
      selectedText,
      metadata,
  ...
  return (
    <div className="min-h-screen bg-white pt-12 pb-2">
          ...
          <Card className="w-full max-w-2xl border border-dashed border-slate-200 bg-white/80 p-9 backdrop-blur-sm">
            <div className="space-y-3">
              ...
              <p className="text-xs leading-relaxed text-slate-600">
                Head back to the home page to paste a YouTube link and generate
                highlight reels, searchable transcripts, and AI takeaways.
              </p>
              <div className="pt-1">
                <Link
                  href="/"
            ...
            {!switchingToLanguage && (
              <p className="mt-1.5 text-xs text-slate-500">
                ...
                {loadingStage === "generating" &&
                  `Creating highlight reels... (${elapsedTime} seconds)`}
                {loadingStage === "processing" &&
                  `Processing and matching quotes... (${processingElapsedTime} seconds)`}
              </p>
            )}
          </div>
          <div className="mt-10 w-full max-w-2xl">
            <LoadingContext videoInfo={videoInfo} preview={videoPreview} />
          </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="mt-1.5 text-sm leading-relaxed text-slate-600">
                  {isRateLimitError ? AUTH_LIMIT_MESSAGE : error}
                </p>
              </div>
      ...
      <TranscriptExportDialog
        ...
        onIncludeSpeakersChange={(value) =>
          setIncludeSpeakers(value && hasSpeakerData)
        }
