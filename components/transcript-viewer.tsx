...
export function TranscriptViewer({
  ...
  const [translationsCache, setTranslationsCache] = useState<
    Map<number, string>
  >(new Map());
  const [loadingTranslations, setLoadingTranslations] = useState<Set<number>>(
    new Set(),
  );
  const [translationErrors, setTranslationErrors] = useState<Set<number>>(
    new Set(),
  );
  ...
  const [searchResults, setSearchResults] = useState<
    { segmentIndex: number; startIndex: number; endIndex: number }[]
  >([]);
  ...
  const selectedTopicColor =
    selectedTopicIndex >= 0
      ? getTopicHSLColor(selectedTopicIndex, videoId)
      : null;
  ...
  const requestTranslation = useCallback(
    async (segmentIndex: number) => {
      ...
      if (
        !onRequestTranslation ||
        !translationEnabled ||
        loadingTranslations.has(segmentIndex) ||
        translationsCache.has(segmentIndex)
      ) {
        return;
      }
      ...
      try {
        ...
        const translation = await onRequestTranslation(
          segment.text,
          cacheKey,
          "transcript",
        );
        setTranslationsCache((prev) =>
          new Map(prev).set(segmentIndex, translation),
        );
      } catch (error) {
    ...
    if (
      ...
      if (
        ...
        if (transcript[firstSeg.startSegmentIdx]) {
          ...
          for (
            let i = Math.max(0, firstSeg.startSegmentIdx - 5);
            i <= Math.min(firstSeg.startSegmentIdx + 5, transcript.length - 1);
            i++
          ) {
            const segText = transcript[i]?.text || "";
            const segTextNorm = segText
              .toLowerCase()
              .replace(/[^a-z0-9 ]/g, "");
  ...
  useEffect(() => {
    if (autoScroll && currentSegmentRef.current && currentTime > 0) {
      ...
      if (viewport) {
        ...
        const bottomThreshold = viewportRect.top + viewportRect.height * 0.4;
  ...
  const getHighlightedText = (
    ...
    const segmentSearchResults = searchResults.filter(
      (r) => r.segmentIndex === segmentIndex,
    );
    ...
    for (const highlightSeg of segmentsToHighlight) {
      ...
      if (
        highlightSeg.startSegmentIdx !== undefined &&
        highlightSeg.endSegmentIdx !== undefined
      ) {
        // Skip this debug logging - removed for cleaner output

        // Skip segments that are before the start or after the end
        if (
          segmentIndex < highlightSeg.startSegmentIdx ||
          segmentIndex > highlightSeg.endSegmentIdx
        ) {
          continue;
        }
        ...
        if (
          ...
          return {
            highlightedParts: [
              { text: segment.text, highlighted: true, isCitation },
            ],
          };
        }
        ...
        if (segmentIndex === highlightSeg.startSegmentIdx) {
          if (
            highlightSeg.startCharOffset !== undefined &&
            highlightSeg.startCharOffset > 0
          ) {
            // Partial highlight from character offset to end
            const beforeHighlight = segment.text.substring(
              0,
              highlightSeg.startCharOffset,
            );
            const highlighted = segment.text.substring(
              highlightSeg.startCharOffset,
            );
            ...
            if (
              ...
              if (beforeHighlight)
                parts.push({ text: beforeHighlight, highlighted: false });
              if (actualHighlighted)
                parts.push({
                  text: actualHighlighted,
                  highlighted: true,
                  isCitation,
                });
              if (afterHighlight)
                parts.push({ text: afterHighlight, highlighted: false });
              return { highlightedParts: parts };
            }
            ...
            if (beforeHighlight)
              parts.push({ text: beforeHighlight, highlighted: false });
            if (highlighted)
              parts.push({ text: highlighted, highlighted: true, isCitation });
            return { highlightedParts: parts };
          } else {
        ...
        if (
          ...
          if (
            highlightSeg.endCharOffset !== undefined &&
            highlightSeg.endCharOffset < segment.text.length
          ) {
    ...
    const hasAnySegmentIndices = segmentsToHighlight.some(
      (seg) =>
  ...
  return (
    <ErrorBoundary>
      <TooltipProvider delayDuration={300}>
        {/* translate="no" 和 notranslate 类名防止浏览器翻译插件修改 DOM 导致 React 崩溃 */}
        <div
          className="h-full max-h-full flex flex-col overflow-hidden notranslate"
          translate="no"
        >
          {/* Header */}
            ...
            >
              <SelectionActions
                ...
                getMetadata={(range) => {
                  ...
                  const segmentElement = startNode?.closest(
                    "[data-segment-index]",
                  ) as HTMLElement | null;
                  if (segmentElement) {
                  ...
                  return transcript.map((segment, index) => {
                    ...
                    return (
                      <div
                        ...
                        ref={(el) => {
                          ...
                          if (el) {
                            if (
                              hasHighlight &&
                              !highlightedRefs.current.includes(el)
                            ) {
                      ...
                      >
                        ...
                        >
                          {highlightedText
                            ? highlightedText.highlightedParts.map(
                                (part, partIndex) => {
                                  const isSearchMatch =
                                    "isSearchMatch" in part &&
                                    part.isSearchMatch;
                                  const isCurrentSearchMatch =
                                    "isCurrentSearchMatch" in part &&
                                    part.isCurrentSearchMatch;
                                  const isCitation =
                                    "isCitation" in part && part.isCitation;
                          ...
                          <div className="flex items-start gap-2">
                            ...
                            >
                              {isLoadingTranslation ? (
                                ...
                                <span className="text-red-500/70 italic text-xs">
                                  Translation failed
                                </span>
                              ) : translation ? (
                                translation
